elements = {  
   ...
   "Ununtrium":{  
      ...
      "period":1,
      "phase":"Solid",
      "summary":"Ununtrium is a chemical element with atomic number 113. It has a temporary name and temporary symbol Uut. It is a synthetic element (an element that can be created in a laboratory but is not found in nature) and is extremely radioactive; its most stable known isotope, ununtrium-286, has a half-life of 20 seconds.",
      "xpos":14,
      "boil":1430
   },
   "Cobalt":{  
   ...
   "Ununpentium":{  
      ...
      "period":1,
      "phase":"Solid",
      "summary":"Ununpentium is the temporary name of a synthetic superheavy element in the periodic table that has the temporary symbol Uup and has the atomic number 115. It is an extremely radioactive element; its most stable known isotope, ununpentium-289, has a half-life of only 220 milliseconds. It is also known as eka-bismuth or simply element 115.",
      "xpos":16,
      "boil":1400
   },
   "Vanadium":{  
...
def getElement(name):
    ...
    for key in elements:
        if key.lower().startswith(name) or elements[key]["number"]==name:
            part1 = "{0} ({1})".format(key,elements[key]["symbol"])
            data = []
            for key2 in ["named_by","color","discovered_by","appearance","number","phase","category"]:
                data.append("\x02{0}: \x0f".format(key2.replace("_"," ").title()) + str(elements[key][key2]))
            data.append("\x02Density: \x0f {0} g/cm^3".format(elements[key]["density"]))
            data.append("\x02Molar Heat: \x0f {0} J/(mol*K)".format(elements[key]["molar_heat"]))
            data.append("\x02Melting: \x0f {0} K".format(elements[key]["melt"]))
            data.append("\x02Boiling: \x0f {0} K".format(elements[key]["boil"]))
            data.append("\x02Atomic Mass: \x0f {0}".format(elements[key]["atomic_mass"]))
            data.append("\x02Wiki: \x0f {0}".format(elements[key]["source"]))
            
            returned = part1 + " " + " | ".join(data)
            return returned

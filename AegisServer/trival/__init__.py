...
def periodic_table(args,user="",hostmask="",extra={}):
    for i in periodic.split("\n"):
...
def ascii(args,user="",hostmask="",extra={}):
    if args == "popcorn":
        for i in popcorn.split("\n"):
            extra["ircsock"].sendmsg(extra["channel"],i)
    elif args == "icecream" or args == "ice cream":
        for i in icecream.split("\n"):
            extra["ircsock"].sendmsg(extra["channel"],i)
    elif args == "coffee":
        for i in coffee.split("\n"):
            extra["ircsock"].sendmsg(extra["channel"],i)
    elif args == "coffee2":
        for i in coffee2.split("\n"):
            extra["ircsock"].sendmsg(extra["channel"],i)
    elif args == "nuke":
        for i in nuke.split("\n"):
            extra["ircsock"].sendmsg(extra["channel"],i)
        ...
        for i in computer.split("\n"):
            extra["ircsock"].sendmsg(extra["channel"],i)
    elif args == "aperture":
        for i in aperture.split("\n"):
            extra["ircsock"].sendmsg(extra["channel"],i)
    else:
        return "Unknown art, choose from: popcorn, icecream, coffee, coffee2, nuke, tank, computer, aperture"
...
def moo(args,user="",hostmask="",extra={}):
    returned = "\x02{0}: ".format(user.replace(":","",1))
    for i in list("Mooooooooooooooooooooooooooooooooooooooooooooooooing!"):
        returned = returned + "\x03{},{}".format(random.randint(0,15),random.randint(0,15)) + i
    return returned

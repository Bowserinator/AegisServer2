...
import convert
import os, requests, time, pyimgur
 
#LAtex rendering
def formula_as_file(args,user="",hostmask="",extra={}):
    if args.replace(" ","") == "": 
        return "No equation found."
    formula = args; file = "calc/" + str(time.time())+".png"
    negate = False;
    tfile = file
    if negate:
        tfile = 'tmp.png'
    r = requests.get( 'http://latex.codecogs.com/png.latex?\dpi{300} \huge %s' % formula )
    if open("calc/invalid.gif","rb").read() == r.content:
        return "Invalid equation found."
    open(tfile, "w").close() #Create the file
    f = open( tfile, 'wb+' )
    f.write(r.content )
    f.close()
    if negate:
    ...
    CLIENT_ID = "fb21b40ecc7426c"
    im = pyimgur.Imgur(CLIENT_ID)
    uploaded_image = im.upload_image(file, title="Generated with AegisServer LaTeX")
    os.remove(tfile)
    return "Your file can be found at: " + uploaded_image.link
...
def convert_stuff(args,user="",hostmask="",extra={}):
    ...
    try: return "\x02Conversion: \x0f" + str(convert.convert(value,unit1,unit2))
        ...
        #return "{0} {1} {2}".format(value,unit1,unit2)

def list_units(args,user="",hostmask="",extra={}):
    catagories = {
        "distance":convert.conversionDis,
        "data":convert.conversionData,
        "time":convert.conversionTime,
        "angle":convert.conversionAngle,
        "mass":convert.conversionMass,
        "weight":convert.conversionMass
    }
    for key in catagories:
        if key == args.lower():
            returned = []
            for i in catagories[key]: returned.append(i.names[0])
            return "\x02Units: \x0f" + ", ".join(returned)
    return "\x02Categories for list: \x0f" + ", ".join(catagories.keys())

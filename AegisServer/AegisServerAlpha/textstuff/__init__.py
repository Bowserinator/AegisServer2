
contractions = {
    "ain't":"am not",
    "amn't":"am not",
    "aren't":"are not",
    "can't":"can not",
    "cannot":"can not",
    "could've":"could have",
    "couldn't":"could not",
    "couldn't've":"could not have",
    "didn't":"did not",
    "doesn't":"does not",
    "don't":"do not",
    "gonna":"going to",
    "hadn't":"had not",
    "hadn't've":"had not have",
    "hasn't":"has not",
    "haven't":"have not",
    "he'd":"he had",
    "he'd've":"he would have",
    "he'll":"he will",
    "he's":"he is",
    "he'sn't":"he has not",
    "how'd":"how did",
    "how'll":"how will",
    "how's":"how is",
    "i'd":"I had",
    "i'd've":"I would have",
    "i'll":"I will",
    "i'm":"I am",
    "i've":"I have",
    "i'ven't":"I have not",
    "isn't":"is not",
    "it'd":"it had",
    "it'd've":"it would have",
    "it'll":"it will",
    "it's":"it is",
    "it'sn't":"it is not",
    "let's":"let us",
    "ma'am":"madam",
    "mightn't":"might not",
    "mightn't've":"might not have",
    "might've":"might have",
    "mustn't":"must not",
    "must've":"must have",
    "needn't":"need not",
    "not've":"not have",
    "o'clock":"of the clock",
    "ol'":"old",
    "oughtn't":"ought not",
    "shan't":"shall not",
    "she'd":"she had",
    "she'd've":"she would have",
    "she'll":"she will",
    "she's":"she is",
    "she'sn't":"she has not",
    "should've":'should have',
    "shouldn't":"should not",
    "shouldn't've":"should not have",
    "somebody'd":"somebody had",
    "somebody'd've":"somebody would have",
    "somebody'dn't've":"somebody would not have",
    "somebody'll":"somebody shall",
    "somebody's":"somebody has",
    "someone'd":"someone had",
    "someone'd've":"someone would have",
    "someone'll":"someone shall",
    "someone's":"someone has",
    "something'd":"something had",
    "something'd've":"something would have",
    "something'll":"something shall",
    "something's":"something has",
    "that'll":"that will",
    "that's":"that has",
    "there'd":"there had",
    "there'd've":"there would have",
    "there're":"there are",
    "there's":"there has",
    "they'd":"they had",
    "they'dn't":"they would not",
    "they'dn't've":"they would not have",
    "they'd've":"they would have",
    "they'd'ven't":"they would have not",
    "they'll":"they shall",
    "they'lln't've":"hey will not have",
    "they'll'ven't":"they will have not",
    "they're":"they are",
    "they've":"they have",
    "they'ven't":"they have not",
    "'tis":"it is",
    "'twas":"it was",
    "wanna":"want to",
    "wasn't":"was not",
    "we'd":"we had",
    "we'd've":"we would have",
    "we'dn't've":"we would not have",
    "we'll":"we will",
    "we'lln't've":"we will not have",
    "we're":"we are",
    "we've":"we have",
    "weren't":"were not",
    "what'll":"what shall",
    "what're":"what are",
    "what's":"what is",
    "what've":"what have",
    "when's":"when has",
    "where'd":"where did",
    "where's":"where has",
    "where've":"where have",
    "who'd":"who would",
    "who'd've":"who would have",
    "who'll":"who shall",
    "who're":"who are",
    "who's":"who is",
    "who've":"who have",
    "why'll":"why will",
    "why're":"why are",
    "why's":"why is",
    "won't":"will not",
    "won't've":"will not have",
    "would've":"would have",
    "wouldn't":"would not",
    "wouldn't've":"would not have",
    "y'all":"you all",
    "y'all'd've":"you all would have",
    "y'all'dn't've":"you all would not have",
    "y'all'll":"you all will",
    "y'all'lln't":"you all will not",
    "y'all'll've":"you all will have",
    "'all'll'ven't":"you all will have not",
    "you'd":"you would",
    "you'd've":"you would have",
    "you'll":"you shall",
    "you're":"you are",
    "you'ren't":"you are not",
    "you've":"you have",
    "you'ven't":"you have not",
    "moar":"more",
    "gawd":"god",
    "thx":"thanks",
}

def replaceShort(text):
    #Replaces shortened stuff like its and they've
    text = " " + text.lower() + " "
    for key in contractions:
        text = text.replace(" " +key+ " "," " +contractions[key]+ " ")
        if key != "he'll" and key != "we'll" and key != "we're" and key != "she'll" and key != "it's":
            text = text.replace(" " +key.replace("'","")+ " "," " +contractions[key]+ " ")
    return text.lstrip().rstrip()

def checkYour(text):
    #http://www.elearnenglishlanguage.com/blog/english-mistakes/your-vs-youre/
    #Returns correct form of you're/your
    pass